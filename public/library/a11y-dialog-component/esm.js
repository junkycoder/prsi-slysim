...
class Dialog {
  constructor(
    ...
    this.document =
      document.querySelector(this.config.documentSelector) ||
    ...
    this.observer = new MutationObserver((mutations) =>
      mutations.forEach(() => this[setFocusableElements]())
    );
    ...
    switch (event.key) {
      ...
      case keyCodes.f6:
        if (!this.config.isModal)
          !this.config.isTooltip ? this[restoreFocus]() : this.close(event);
        break;
      case keyCodes.tab:
  ...
  [addEventDelegation](event) {
    document
      .querySelectorAll(this.config.openingSelector)
      .forEach((openingTrigger) => {
    ...
    document
      .querySelectorAll(this.config.closingSelector)
      .forEach((closingTrigger) => {
  ...
  [addAttributes]() {
    ...
    if (this.config.labelledby)
      this.dialog.setAttribute("aria-labelledby", this.config.labelledby);
    if (this.config.describedby)
      this.dialog.setAttribute("aria-describedby", this.config.describedby);

    if (this.config.isModal) this.dialog.setAttribute("aria-modal", true);

    this.openingTriggers.forEach((openingTrigger) =>
      openingTrigger.setAttribute("aria-haspopup", "dialog")
    );
  }
  ...
  [removeAttributes]() {
    ...
    if (
      ...
      this.isOpen &&
      !this.documentIsAlreadyDisabled
    ) {
      this.document.classList.remove(this.config.documentDisabledClass);
    }

    this.openingTriggers.forEach((openingTrigger) =>
      openingTrigger.removeAttribute("aria-haspopup")
    );

    if (this.openingTrigger)
      this.openingTrigger.classList.remove(
        this.config.openingTriggerActiveClass
      );
  ...
  [setAttributes]() {
    ...
    if (this.openingTrigger) {
      if (this.isOpen) {
        this.openingTrigger.classList.add(
          this.config.openingTriggerActiveClass
        );
      } else {
        this.openingTrigger.classList.remove(
  ...
  [setFocusableElements]() {
    const visibleFocusableElements = getVisibleElements(
      this.dialog.querySelectorAll(focusableElements)
    );
    const filteredFocusableElements = getNoNestedElements(
      this.dialog,
      '[role="dialog"]',
  ...
  [restoreFocus]() {
    ...
    if (this.isOpen)
      this.openingTrigger.addEventListener("keydown", this[switchFocus]);
  }
  ...
  open() {
    ...
    this.isOpen = true;
    this.documentIsAlreadyDisabled = this.document.classList.contains(
      this.config.documentDisabledClass
    );
  ...
  close(event) {
    ...
    if (
      this.openingTrigger &&
      (!this.config.isTooltip ||
        (this.config.isTooltip && event && event.type !== "click"))
    ) {
      this[restoreFocus]();
    }
